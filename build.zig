...
pub fn getModule(b: *std.Build) *std.Build.Module {
    const module_info = std.Build.CreateModuleOptions{
        .source_file = .{ .path = sdkPath("/src/main.zig") },
    };
    return b.createModule(module_info);
}
...
fn buildLibrary(b: *Build, mode: std.builtin.Mode, target: std.zig.CrossTarget, options: Options) BuildError!*std.build.CompileStep {
